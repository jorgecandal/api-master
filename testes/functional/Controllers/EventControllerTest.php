...
class EventControllerTest extends FunctionalTestCase
{
  public function testAInsertNewEvent() {
    ...
    $data = array('title'       => '8.PHP - Fórum Nacional de Profissionais PHP do MA',
    ...
    $response = $client->request('POST', '/events', [ 'form_params' => $data ]);

    $this->assertEquals(200, $response->getStatusCode());
    $this->assertEquals('Event created with success', json_encode($response->getBody())->msg);
  ...
  public function testUpdateEvent() {
    ...
    $dataUpdate = array('title'       => '8.PHP - Fórum Nacional de Profissionais PHP do MA',
                  'content'     => 'Fórum Nacional de Profissionais PHP - MA',
                  'description' => 'Fórum Nacional de Profissionais PHP - MA',
                  'venue'       => 'LOCAL',
                  'address'     => 'ENDEREÇO',
    ...
    $response = $client->request('PUT', '/events', [ 'form_params' => $dataUpdate ]);

    $this->assertEquals(200, $response->getStatusCode());
    $this->assertEquals('Event updated with success', json_encode($response->getBody())->msg);
  }
  ...
  public function testDeleteEvent() {
    ...
    $this->assertEquals(200, $response->getStatusCode());
    $this->assertEquals('Event deleted with success', json_encode($response->getBody())->msg);
  }
}
